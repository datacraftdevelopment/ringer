...
- **Status:** **built (2026-07-30, on Joe's direct ask — he was the demand the flag waited for).** Lives at `templates/ralph-loop/` (ralph_loop.py + tests + README + example PRD). Machinery proven twice: mock-engine self-test (3/3, zero tokens — scheduling, blocking edges, verdict-driven passes, patch apply, commits, guards, stop conditions) and a live two-lane run (codex built greet.py; haiku extended it in a worktree containing iteration 1's commit; 2 first-try PASSes, 2 clean `ralph:` commits, `<promise>COMPLETE</promise>`). Design deltas from the sketch below: **worktrees mode** instead of direct-repo edits (sandbox-compatible; the harness deletes passing worktrees, so the check exports a patch and the LOOP applies + commits), and run scratch lives under `~/.ringer/ralph/<slug>/`, never in the repo. **Remaining for the upstream PR: the real-backlog proving run** (the cloud-agents experiment) — upstream's bar is "a swarm pattern that *worked* for you" with run evidence.
